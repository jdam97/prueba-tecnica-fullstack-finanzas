...
import { getTransactionsService, createTransactionService, deleteTransactionService } from "../services/transaction.service"
...
export async function createTransactionController(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
      const newTransaction = await createTransactionService({ amount, concept, date, userId, type });
  ...
  export async function deleteTransactionController(req: NextApiRequest, res: NextApiResponse) {
    const { id } = req.query;
    if (typeof id !== "string") {
